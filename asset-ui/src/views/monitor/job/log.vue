    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleClose"
        >{{ $t('common_field.close') }}</el-button>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    /** 清空按钮操作 */
    handleClean() {
      this.$modal.confirm(this.$t('common_field.tip'), this.$t('common_field.confirm'), this.$t('common_field.cancel'),'是否确认清空所有调度日志数据项？').then(function() {
        return cleanJobLog();
      }).then(() => {
        this.getList();
