<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" :label-width="$t('label_width')">
      ...
      <el-form-item :label="$t('dict.label')" prop="dictLabel">
        <el-input
          ...
          :placeholder="$t('common_field.fill') + $t('dict.label')"
          clearable
          size="small"
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column :label="$t('dict.label')" align="center" prop="dictLabel">
        <template slot-scope="scope">
          <span v-if="scope.row.listClass == '' || scope.row.listClass == 'default'">{{scope.row.dictLabel}}</span>
          <el-tag v-else :type="scope.row.listClass == 'primary' ? '' : scope.row.listClass">{{scope.row.dictLabel}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('dict.labelEn')" align="center" prop="dictLabelEn">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('common_field.sort')" align="center" prop="dictSort" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" :label-width="$t('label_medium_width')">
        ...
        <el-form-item :label="$t('dict.label')" prop="dictLabel">
          <el-input v-model="form.dictLabel" :placeholder="$t('common_field.fill') + $t('dict.label')" />
        </el-form-item>
        <el-form-item :label="$t('dict.labelEn')" prop="dictLabelEn">
          <el-input v-model="form.dictLabelEn" :placeholder="$t('common_field.fill') + $t('dict.labelEn')" />
        </el-form-item>
        <el-form-item :label="$t('dict.value')" prop="dictValue">
          <el-input v-model="form.dictValue" :placeholder="$t('common_field.fill') + $t('dict.value')" />
        </el-form-item>
        <el-form-item :label="$t('dict.style')" prop="cssClass">
          <el-input v-model="form.cssClass" :placeholder="$t('common_field.fill') + $t('dict.style')" />
        </el-form-item>
        <el-form-item :label="$t('common_field.sort')" prop="dictSort">
          <el-input-number v-model="form.dictSort" controls-position="right" :min="0" />
        </el-form-item>
        <el-form-item :label="$t('dict.back_style')" prop="listClass">
...
export default {
  ...
  data() {
    return {
      ...
      listClassOptions: [
        {
          value: "default",
          label: this.$t('style.default')
        },
        {
          value: "primary",
          label: this.$t('style.primary')
        },
        {
          value: "success",
          label: this.$t('style.success')
        },
        {
          value: "info",
          label: this.$t('style.info')
        },
        {
          value: "warning",
          label: this.$t('style.warning')
        },
        {
          value: "danger",
          label: this.$t('style.danger')
        }
      ],
      // 类型数据字典
      typeOptions: [],
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = this.$t('dict.insert_data');
    ...
    handleUpdate(row) {
      ...
      getData(dictCode).then(response => {
        ...
        this.open = true;
        this.title = this.$t('dict.update_data');
      });
    },
    /** 提交按钮 */
    submitForm: function() {
