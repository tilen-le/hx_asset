<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" :label-width="$t('label_width')">
      <el-form-item :label="$t('notice.title')" prop="noticeTitle">
        <el-input
          ...
          :placeholder="$t('common_field.fill') + $t('notice.title')"
          clearable
          size="small"
      ...
      <el-form-item :label="$t('notice.operator')" prop="createBy">
        <el-input
          ...
          :placeholder="$t('common_field.fill') + $t('notice.operator')"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item :label="$t('common_field.type')" prop="noticeType">
        <el-select v-model="queryParams.noticeType" :placeholder="$t('common_field.type')" clearable size="small">
          <el-option
    ...
    <el-table v-loading="loading" :data="noticeList" @selection-change="handleSelectionChange">
      ...
      />
      <el-table-column :label="$t('common_field.type')" align="center" prop="noticeType" width="100">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_notice_type" :value="scope.row.noticeType"/>
        </template>
      </el-table-column>
      <el-table-column :label="$t('common_field.status')" align="center" prop="status" width="100">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_notice_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column :label="$t('common_field.create_by')" align="center" prop="createBy" width="100" />
      <el-table-column :label="$t('common_field.create_time')" align="center" prop="createTime" width="100">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="780px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" :label-width="$t('label_medium_width')">
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('notice.title')" prop="noticeTitle">
              <el-input v-model="form.noticeTitle" :placeholder="$t('common_field.fill') + $t('notice.title')" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('common_field.type')" prop="noticeType">
              <el-select v-model="form.noticeType" :placeholder="$t('common_field.select') + $t('common_field.type')">
                <el-option
                  v-for="dict in dict.type.sys_notice_type"
                  :key="dict.value"
                  :label="dict.label"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        noticeType: [
          { required: true, message: this.$t('common_field.not_null'), trigger: "change" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = this.$t('notice.insert');
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getNotice(noticeId).then(response => {
        ...
        this.open = true;
        this.title = this.$t('notice.update');
      });
    },
    /** 提交按钮 */
    submitForm: function() {
