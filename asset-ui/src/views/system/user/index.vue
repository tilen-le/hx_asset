  ...
  <div class="app-container">
      ...
      <el-col :span="20" :xs="24">
        ...
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            ...
            >{{$t('common_field.import')}}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              type="warning"
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column :label="$t('common_field.name')" align="center" key="userName" prop="userName" v-if="columns[0].visible" :show-overflow-tooltip="true" />
          <el-table-column :label="$t('user.nickname')" align="center" key="nickName" prop="nickName" v-if="columns[1].visible" :show-overflow-tooltip="true" />
          <el-table-column :label="$t('user.dept')" align="center" key="deptName" prop="dept.deptName" v-if="columns[2].visible" :show-overflow-tooltip="true" />
          <el-table-column :label="$t('common_field.phone')" align="center" key="phonenumber" prop="phonenumber" v-if="columns[3].visible" width="120" />
          <el-table-column :label="$t('common_field.status')" align="center" key="status" v-if="columns[4].visible">
          ...
          <el-table-column :label="$t('common_field.create_time')" align="center" prop="createTime" v-if="columns[5].visible" width="160">
            <template slot-scope="scope">
              <span>{{ parseTime(scope.row.createTime) }}</span>
            </template>
          </el-table-column>
          <el-table-column
            :label="$t('common_field.operate')"
            align="center"
            width="160"
              ...
              <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['system:user:resetPwd', 'system:user:edit']">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item command="handleResetPwd" icon="el-icon-key"
                    v-hasPermi="['system:user:resetPwd']">{{ $t('user.reset_password') }}</el-dropdown-item>
                  <el-dropdown-item command="handleAuthRole" icon="el-icon-circle-check"
                    v-hasPermi="['system:user:edit']">{{ $t('user.select_role') }}</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="650px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('user.nickname')" prop="nickName">
              <el-input v-model="form.nickName" :placeholder="$t('common_field.fill') + $t('user.nickname')" maxlength="30" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('user.dept')" prop="deptId">
              <treeselect v-model="form.deptId" :options="deptOptions" :show-count="true" :placeholder="$t('common_field.fill') + $t('user.dept')" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('common_field.phone')" prop="phonenumber">
              <el-input v-model="form.phonenumber" :placeholder="$t('common_field.fill') + $t('common_field.phone')" maxlength="11" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item v-if="form.userId == undefined" :label="$t('common_field.name')" prop="userName">
              <el-input v-model="form.userName" :placeholder="$t('common_field.fill') + $t('common_field.name')" maxlength="30" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item v-if="form.userId == undefined" :label="$t('user.password')" prop="password">
              <el-input v-model="form.password" :placeholder="$t('common_field.fill') + $t('user.password')" type="password" maxlength="20" show-password/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('user.sex')">
              <el-select v-model="form.sex" :placeholder="$t('common_field.select')">
                <el-option
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('user.post')">
              <el-select v-model="form.postIds" multiple :placeholder="$t('common_field.select')">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item :label="$t('user.role')">
              <el-select v-model="form.roleIds" multiple :placeholder="$t('common_field.select')">
                <el-option
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body>
      ...
      >
        ...
        <div class="el-upload__tip text-center" slot="tip">
          ...
          <el-link type="primary" :underline="false" style="font-size:12px;vertical-align: baseline;" @click="importTemplate">{{$t('common_field.download_template')}}</el-link>
        </div>
      </el-upload>
      <div slot="footer" class="dialog-footer">
...
<script>
  ...
  data() {
    return {
      ...
      // 列信息
      columns: [
        { key: 0, label: this.$t('common_field.name'), visible: true },
        { key: 1, label: this.$t('user.nickname'), visible: true },
        { key: 2, label: this.$t('user.dept'), visible: true },
        { key: 3, label: this.$t('common_field.phone'), visible: true },
        { key: 4, label: this.$t('common_field.status'), visible: true },
        { key: 5, label: this.$t('common_field.create_time'), visible: true }
      ],
      // 表单校验
      rules: {
        userName: [
          { required: true, message: this.$t('common_field.not_null'), trigger: "blur" },
          { min: 2, max: 20, message: this.$t('user.name_format'), trigger: 'blur' }
        ],
        nickName: [
          { required: true, message: this.$t('common_field.not_null'), trigger: "blur" }
        ],
        password: [
          { required: true, message: this.$t('common_field.not_null'), trigger: "blur" },
          { min: 5, max: 20, message: this.$t('user.password_format'), trigger: 'blur' }
        ],
        email: [
          {
            type: "email",
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getUser(userId).then(response => {
        ...
        this.open = true;
        this.title = this.$t('user.update');
        this.form.password = "";
      });
    },
    /** 重置密码按钮操作 */
    handleResetPwd(row) {
      let title = this.$t('common_field.fill') + this.$t('user.new_password');
      let tip = this.$t('common_field.tip');
      let confirm = this.$t('common_field.confirm');
      let cancel = this.$t('common_field.cancel');
      let format = this.$t('user.password_format');
      this.$prompt(title, tip, {
        confirmButtonText: confirm,
        cancelButtonText: cancel,
        closeOnClickModal: false,
        inputPattern: /^.{5,20}$/,
        inputErrorMessage: format
      }).then(({ value }) => {
